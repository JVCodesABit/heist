...
const EVIDENCE_ORDER: EvidenceType[] = ['whatsapp', 'transactions', 'contract', 'website', 'bankstatement'];
...
export default function EvidenceViewer() {
  ...
  const nextType = currentIdx < 4 ? EVIDENCE_ORDER[currentIdx + 1] : null;
    ...
    <div className="flex h-[calc(100vh-52px)] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6">
        ...
        <div className="font-mono text-[11px] text-text-tertiary mb-4">
          CASE #047 → EVIDENCE → {EVIDENCE_LABELS[type]?.toUpperCase()}
        </div>

        {/* Nav */}
        <div className="flex items-center justify-between mb-6">
          <button
            onClick={() => prevType && navigate(`/evidence/${prevType}`)}
            disabled={!prevType}
          ...
          <button
            onClick={() => nextType && navigate(`/evidence/${nextType}`)}
            disabled={!nextType}
            className="btn-ghost font-typewriter text-xs disabled:opacity-30"
          >
            NEXT →
